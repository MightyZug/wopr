...
class GameStateManager:
    ...
    def __init__(self):
        ...
        self.russia_destroyed: List[bool] = [False] * len(RUSSIA_CITIES)
        self.us_cities_destroyed: List[str] = []
        self.russian_cities_destroyed: List[str] = []
    ...
    def start_new_game(self):
        ...
        self.russia_destroyed = [False] * len(RUSSIA_CITIES)
        self.us_cities_destroyed = []
        self.russian_cities_destroyed = []
        self.missile_lines = []
    ...
    def toggle_target(self, city_index: int) -> bool:
        """Toggle target selection for a Russian city. Returns True if successful."""
    ...
    def make_ai_selections(self):
        ...
        self.ai_defenses = set(random.sample(range(len(RUSSIA_CITIES)), DEFENSE_LIMIT))
    ...
    def calculate_casualties(self) -> tuple:
        ...
        russia_casualties = sum(RUSSIA_CITIES[idx]["population"] 
                               for idx in range(len(RUSSIA_CITIES)) 
                               if self.russia_destroyed[idx])
        
        total_us_population = sum(city["population"] for city in USA_CITIES)
        total_russia_population = sum(city["population"] for city in RUSSIA_CITIES)
        
        us_casualty_percent = (us_casualties / total_us_population) * 100 if total_us_population > 0 else 0
        russia_casualty_percent = (russia_casualties / total_russia_population) * 100 if total_russia_population > 0 else 0
        
        return (us_casualties, russia_casualties, total_us_population, 
                total_russia_population, us_casualty_percent, russia_casualty_percent)
